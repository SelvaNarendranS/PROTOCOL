// I2c slave - only for write(receive only)
...
module i2c_slave #(
  ...
  reg [DATA_WIDTH-1:0] 		data_sft_reg;	// data shift register
      ...
      case(state)
        ...
        ADDR_ACK : begin
          if(scl_falling) begin		// sending ack bit at posedge scl clk
            ...
            if(addr_sft_reg[ADDRESS_WIDTH:1] == SLAVE_ADDR[ADDRESS_WIDTH-1:0]) begin
              if(addr_sft_reg[0]) begin
                state   <= IDLE;		// address mismatch so returning to default state
              end
              else begin
        ...
        WRITE : begin
          if(scl_rising) begin	// scl rising edge -- sampling the data bits
            data_sft_reg[data_count] <= sda_level;	// sampling synchronizer data bits
