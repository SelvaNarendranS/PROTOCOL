// I2c master -- write and read operation
...
module i2c_master #(
  ...
  (
    ...
    output reg[DATA_WIDTH-1:0] data_out,    // data received(read)
  ...
  reg [DATA_WIDTH-1:0] 		tx_data_sft_reg;	// input(transfer) data shift register
  reg [DATA_WIDTH-1:0] 		rx_data_sft_reg;	// output(receiver) data shift register
  ...
  always @(posedge clk or negedge rst) begin
    if(!rst) begin			// active low reset
      ...
      case(state)
        ...
        ADDR_ACK : begin
          ...
          if(scl_rising) begin			// at previous scl falling edge(low) and present scl should be high sample ack from slave
            if(sda) begin			// if ack(sda line) == 1 -- error in address received
              ...
              state <= (rw) ? READ : WRITE;		// received ack
        ...
        WRITE : begin
          if(scl_falling) begin			// check previous clk high -- now present clk low(transmit happen)
            if(data_count == 0) begin
              ...
              sda_out <= tx_data_sft_reg[data_count-1];	// data bit by bit transmission on negedge scl clk
        ...
        // -------------- READ state -------------------
        READ : begin
          sda_out <= 1;				// release sda to make slave to transmit data
          if(scl_rising) begin
            rx_data_sft_reg <= {rx_data_sft_reg[DATA_WIDTH-2:0], sda};		// sampling
        ...
        READ_ACK : begin
          if(scl_falling) begin
            sda_out <= 1;			// NACK single byte received
          end
          else if(scl_rising) begin
            sda_out <= 1;		// release sda line
            data_out <= rx_data_sft_reg;		// latching received data
            state <= STOP;
          end
        end
